...
# Configuration
QUERY = 'all:6G AND (all:wireless OR all:network)'
MAX_RESULTS = 5
...
def update_readme(papers):
    ...
    # Insert after the "Academic Research" header if it exists, otherwise append
    marker = "## Academic Research"
    if marker in content:
        updated_content = content.replace(marker, marker + new_content)
    else:
        # Fallback: create the section at the top
